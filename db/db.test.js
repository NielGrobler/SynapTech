...
describe('Database Module Tests', () => {
    ...
    describe('fetchAssociatedProjects', () => {
        it('should fetch all projects associated with a user', async () => {
            const user = { id: 1 };
            const projects = await db.fetchAssociatedProjects(user);
    ...
    describe('appendCollaborators', () => {
        it('should append collaborators to each project', async () => {
            const projects = [
            ...
            expect(projects[0].collaborators).toHaveLength(2);
            expect(projects[0].collaborators[0].name).toBe('Collaborator 1');
            // Note: The mock currently returns the *same* collaborators for any project ID queried.
            // You might need more specific mock logic if appendCollaborators queries differently per project.
            expect(projects[1].collaborators).toBeDefined();
            expect(projects[1].collaborators).toHaveLength(2);
        });
    });
