...
import { BadUploadTypeError } from './errors.js';

const port = process.env.PORT || 3000;

const createIo = (routerInstance) => {
	...
	io.on('connection', (socket) => {
		...
		socket.on('message-with-attachment', async ({ roomId, text, attachment }) => {
			...
			try {
				...
				console.log(error);
				if (error instanceof BadUploadTypeError) {
					socket.emit('error', { message: `${attachment.name} is not permitted.` });
				} else {
